...
An unread failed run shows **Run failed** on its own row. When a failed child session needs attention, its ancestors instead show **Child run failed** with the failing session's name and reason; this does not mean the parent run failed. Expand the child disclosure to inspect the failure. Reading the failed child dismisses that warning, including its copies on ancestor rows.
