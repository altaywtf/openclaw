...
| `readyWorkers`       | Target unassigned prepared workers per eligible project using this profile; default `1`. Must be a nonnegative integer. Set `0` to disable this profile's reserves while preserving snapshot reuse. Subject to the Gateway-wide `cloudWorkers.preparedPool.maxTotal` cap, which defaults to `4`. See [Ready workers](/gateway/cloud-workers#ready-workers).                                                                                                                                                                    |
...
After a successful eligible project dispatch, OpenClaw prepares fresh workers in the background for later sessions. A ready worker has its project checkout, committed setup output, current runtime, node enrollment, and required worker bundle already prepared. The next matching session claims it once, applies its current workspace changes, and activates through the existing node connection. A first dispatch, an expired reserve, or a burst that consumes every ready worker still uses ordinary provisioning.

The pool belongs to the Gateway. Crabbox owns machine and snapshot operations through its configured direct-provider or coordinator path; a Crabbox coordinator is not required for the pool itself. Reserve preparation requires a dedicated node, reusable project preparation, and a provider that supplies an idle policy. Shared paired devices, SSH-only profiles, and profiles with nonempty mutable `settings.setupEnv` do not receive ready reserves.

Use `cloudWorkers.profiles.<id>.readyWorkers` to set the target per project using that profile, and `cloudWorkers.preparedPool.maxTotal` for the Gateway-wide cap across projects and profiles. Defaults are `1` and `4`. Edit these values in `openclaw.json` or **Settings → Advanced**. Both accept nonnegative integers; setting either to `0` disables reserves at that scope. Lowering a limit drains excess unassigned workers without stopping assigned sessions. Snapshot reuse remains available. Use separate profiles and [project defaults](/gateway/cloud-workers#per-project-default-profiles) when projects need different targets.

Preparing workers and reserve workers awaiting confirmed cleanup count against both limits. OpenClaw prepares at most two workers concurrently. A changed commit, setup recipe, runtime artifact, enabled plugin content, execution mode, or effective machine class invalidates the old preparation; replacement generations do not each get another slot. A used worker never returns to the reserve, including after failed attachment or a Gateway restart.

Ready workers incur running-machine charges. Their fixed expiry starts at the actual project demand that triggered preparation and uses the provider's existing idle policy: `settings.idleTimeout` for Crabbox. This is separate from `suspendAfter`, which applies to an assigned idle session. Refills, retries, inspections, and heartbeats do not extend reserve expiry. A failed dispatch does not renew demand; replacement capacity can still use the previous successful dispatch's unexpired window. Periodic maintenance requests teardown even without another dispatch; uncertain deletion remains recorded and consumes capacity. A Gateway or cleanup-owner outage can delay physical deletion and continue charges, so provider TTL settings are not a uniform guarantee of autonomous cleanup across backends.
...
OpenClaw deletes unused, unpinned images after 14 days and reclaims the least recently demanded eligible image before admitting a 129th profile record. Actual session demand, including consumption of an already-ready worker, refreshes the exact image generation's demand timestamp. Background replenishment, retries, and verification do not keep an unused image alive. Periodic maintenance runs without another dispatch. Provider deletion must succeed before its ownership record is removed. Pending captures, retirements, and outstanding allocations retain their slots; retirement also waits for allocations using that checkpoint to stop. If all 128 slots are retained, new warm-image allocations fail with cleanup guidance. Each profile record admits at most 256 outstanding allocations and owns at most its current image plus one capture or predecessor retirement. Capacity never evicts a retry choice or cleanup obligation.
...
Before capture, OpenClaw removes per-lease worker identities, device tokens, and session state, including node-host workspaces and SSH-transport workspaces under `~/.openclaw-worker/workspaces`. Machine-level caches intentionally survive: npm caches, content-addressed node runtime and worker bundle installs under `~/.openclaw-worker`, and pristine Git seeds under `~/.openclaw-worker/git-seeds`. A prepared project also retains its fixed workspace and credential-free setup output; each fresh clone registers that workspace to its own environment before use. Project preparation supplies only immutable committed inputs; current session changes arrive after enrollment. Images also retain whatever `settings.setup` wrote elsewhere, so keep setup credential-free and enable reuse only for mutually trusted workloads.

Scrubbing has a three-minute timeout. Checkpoint creation requests `--wait --wait-timeout 45m`, matching Crabbox's native snapshot wait budget, so the exact capture settles before enrollment. The command allows another three minutes for submission and CLI overhead. It also covers Crabbox's separate source lifecycle windows: three minutes for Daytona recovery, or two default 15-minute windows for Machine0 stop and restoration. Provisioning and teardown include these phases and process settlement; inspection and scrub limits stay short. These are finite limits, not expected durations or another capture attempt. They do not extend `ttl` or `idleTimeout`; size the lease lifetime for initial setup and capture as well as useful session work.

Scrub failure releases only its own capture reservation. Once creation starts, failure, timeout, or unusable output leaves its outcome uncertain: the profile stays paused until explicit recovery. An unresolved project capture prevents node enrollment on that source, so fresh node credentials cannot enter a capture that may still be running. Lease cleanup still runs, and a retained usable image can serve new allocations. Capture needs a Crabbox CLI and backend that support fixed-ID checkpoint forks. Continuing a coordinator-retained `checkpoint_pending` response requires the CLI repair in [Crabbox #1698](https://github.com/openclaw/crabbox/pull/1698); older binaries can accept `--wait` and still fail on that response. Correct missing capabilities or permissions before recovering an uncertain capture.

A snapshot fork provisions a fresh lease with fresh node enrollment. Ready reserves complete this work before the next session needs them. Cold allocations and snapshot forks use the same configured lease lifetime, idle timeout, desktop setting, and public networking without Tailscale. Snapshot reuse does not resume an earlier session or its processes.
...
Warm profiles use a version-3 envelope in the existing `warm-images` plugin-state namespace. Ready-worker lifecycle facts and node workspace bindings use state SQLite schema version 16; the per-agent schema is unchanged. Before upgrading, stop the owning Gateway and original capture processes and take a verified backup that includes committed WAL data. Then run:

```bash
openclaw doctor --fix
```

Doctor performs this migration under the Gateway's exclusive maintenance lock. It preserves checkpoint identities, fixed allocation choices, capture selectors, source leases, and retirement obligations. Legacy images without a proven preparation fingerprint remain unverified; historical last-use timestamps do not become proven demand. Such images remain recorded while pinned and retire through ordinary cleanup when unpinned, rather than serving new prepared allocations. Older empty capture markers become explicitly uncertain captures with their original recovery selector. Unsupported records stay unchanged and produce a warning. Runtime provisioning requires the canonical envelope; it does not silently convert old rows.

Older Gateway and node builds refuse the newer state schema. Downgrade requires confirmed resource cleanup by a compatible owner, followed by restoring the verified pre-upgrade backup; changes after that backup are not preserved. Do not lower schema markers or run an older writer against upgraded state. See [Database schemas](/reference/database-schemas) for migration and backup requirements.
...
Changes under `cloudWorkers`, including reserve limits, require a Gateway restart. The default `gateway.reload.mode: "hybrid"` watches the config and performs that restart automatically; with reload watching disabled, run `openclaw gateway restart`.
...
`sessions.dispatch` closes local turn admission, drains active work, and validates the eligible Git workspace inventory. It first tries to claim a matching ready worker. A hit keeps that worker's existing environment and node identity, mints fresh session authority, applies the session workspace, and activates without another provider allocation or enrollment. On a miss, dispatch provisions the lease for the selected execution mode and runs setup. With project warm images enabled, it prepares the committed checkout and node runtime and captures any needed image before enrollment, then installs the required pinned Gateway bundle and applies the session workspace. Both paths return once the placement reaches `active` ownership.

Inventory validation happens before provider allocation and reports an invalid request with an actionable size or entry limit when the workspace cannot be dispatched. Budget several minutes for the first cloud dispatch, including capture when needed; subsequent dispatches can claim a ready reserve or reuse the image, project seed, and runtime installs during provisioning. Workspace changes and current authorization still govern activation; a ready pool does not guarantee a fixed startup time for arbitrary transfers or concurrent bursts. After dispatch, talk to the session as usual. OpenClaw turns route to the worker process; Codex native operations run on the authorized cloud node, paired device, or supported SSH-backed provider.
...
Remote-exec skill bundles are private, read-only turn inputs beside the execution workspace. They stay outside ordinary Git staging, workspace synchronization, and reconciliation. Normal turn cleanup removes them; cleanup failures are reported. Before preparing the next turn, OpenClaw removes recognized leftover private skill copies for that workspace generation, including copies whose initialization response was lost. This also runs when the new turn selects no skills. Recovery preserves attachments, unrelated directories, and other workspace generations, and a cleanup failure stops preparation with retry guidance.
...
- **A node requires an update before hosting sessions** — the Gateway checks workspace support before starting or restoring a placement. On the affected paired or custom node, run `openclaw update`, then reconnect it; for a headless node, run `openclaw node restart`. Updating only the worker bundle does not update the node host. Existing environment teardown remains available while the node needs an update.
...
- **A later session still provisions a worker** — check the profile's `readyWorkers` target, `preparedPool.maxTotal`, provider eligibility, and whether preparation finished before dispatch. A changed preparation fingerprint, expired reserve, or simultaneous demand can produce a capacity miss. Pending cleanup consumes capacity until deletion is confirmed; raising the limit does not repair unresolved cleanup.
