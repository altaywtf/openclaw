...
<Steps>
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="Common provider hooks">
      ...
      - Prompt contribution hooks receive an optional `trigger`: `background`, `cron`, `heartbeat`, `manual`, `memory`, `overflow`, or `user`. `background` identifies an ordinary event or completion turn; `cron` identifies scheduled work.
