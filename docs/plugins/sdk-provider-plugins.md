...
<Steps>
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="SDK seams powering the family builders">
      ...
      - `openclaw/plugin-sdk/provider-model-capabilities` - private runtime helper for bundled and trusted official provider integrations. Reuse its lightweight model and transport contracts in policy artifacts and stream wrappers instead of maintaining separate support lists.
    ...
    <Accordion title="Common provider hooks">
      ...
      - Bundled and trusted official providers can export `resolveFastModeCapability(ctx)` from package-root `provider-policy-api.ts`. Import `ProviderFastModeCapabilityContext` from their private runtime helper `openclaw/plugin-sdk/provider-model-capabilities`. The host supplies the canonical provider/model pair, resolved API, base URL, endpoint class, runtime, authentication requirement, request-override presence, and effective model parameters. Return `true` only when that route can apply a fast-mode selection, `false` for a known unsupported route, and `undefined` when required facts are missing. Reuse the transport's model contract; a provider name alone does not establish support.
      - The model catalog projects this result as optional `supportsFastMode`. Omission means unknown, not unsupported. Publish it with the same row's runtime and effective fast-mode value. A known-empty `thinkingLevels` array remains empty; missing effort levels or defaults do not authorize clients to create their own.
