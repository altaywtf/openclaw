...
`name` is an optional display name. `autoStart` opts into curated setup under
**Settings → Communications → Meeting capture**. Its only keys are `accountId`,
...
partial setup. Title and custom session ID controls are shared UI fields, not
locator descriptor keys.
