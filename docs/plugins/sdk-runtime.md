...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    Its optional `trigger` identifies the run's purpose to provider prompt hooks. Use `background` for an ordinary event or completion turn and `cron` for scheduled work. Scheduling and delivery remain with the calling owner.
