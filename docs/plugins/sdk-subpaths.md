...
runtime splinters have been removed; bundled-only helpers are private-local.
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-catalog-live-runtime` | Private-local after July 2026; guarded live discovery with `buildLiveModelProviderConfig`, `buildOpenAICompatibleLiveProviderCatalog`, `runLiveProviderCatalog`, provider-owned `projectRows`, row/ID fetch helpers, and `LiveModelCatalogHttpError`. Bundled callers use strict discovery; existing external calls retain the shipped advisory fallback. |
