...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - After a synchronous start (including an already-running service) or restart completes, the CLI checks configured plugin readiness before reporting success. Activated plugin errors and configured plugins that could not load make the command exit nonzero with recovery commands, while the Gateway stays running in degraded mode. External channel probe failures do not fail this lifecycle check. A deferred `gateway restart --safe` reports that the restart was scheduled; it cannot report post-restart plugin readiness synchronously.
