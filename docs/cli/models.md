...
`models auth logout <profileId>` removes one saved auth profile from its owning store. Use the profile id shown by `models auth list`. The shared removal flow first clears references in `auth.profiles`, `auth.order`, and `models.providers.<id>.apiKey`, then removes the credential. An order emptied by this removal is deleted; an already-empty order is preserved. If the config write fails, the credential is kept so removal can be retried. It prompts for confirmation on a TTY; pass `--yes` for scripts and agents. Unknown profiles are rejected. Other credentials, including environment-provided keys, can still provide access.
