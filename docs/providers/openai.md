...
authoritative. A separate custom provider ID can opt in to
[custom native provider binding](/plugins/codex-harness-reference#custom-native-providers)
for a matching Responses API-key route on agent-home local stdio. Adding
...
<Tabs>
  <Tab title="API key (OpenAI Platform)">
    **Best for:** direct API access and usage-based billing.
    ...
    Separate custom provider IDs use the explicit
