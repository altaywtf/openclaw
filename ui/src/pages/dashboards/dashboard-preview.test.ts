...
import { dashboardPreviewWidgets, type DashboardPreviewLoader } from "./dashboard-preview.ts";
...
async function mount(
  props: Partial<Pick<PreviewElement, "sessionKey" | "agentId" | "version" | "load">>,
): Promise<PreviewElement> {
  const element = document.createElement("openclaw-dashboard-preview") as PreviewElement;
  Object.assign(element, { sessionKey: snapshot.sessionKey, version: 1 }, props);
...
describe("dashboard preview", () => {
  ...
  it("lays out the first tab's widgets on the board grid", () => {
    const widgets = dashboardPreviewWidgets(snapshot);
    expect(widgets.map((entry) => [entry.name, entry.x, entry.y, entry.w, entry.h])).toEqual([
      ["revenue", 0, 0, 8, 4],
      ["errors", 8, 0, 4, 4],
    ]);
    expect(widgets.map((entry) => entry.title)).toEqual([
      "Revenue by month, including refunds, chargebacks, and regional adjustments",
      "Error rate",
    ]);
    expect(widgets[1]?.kind).toBe("plugin");
  });
  ...
  it("reloads only when the row change marker moves", async () => {
    const load = vi.fn(async () => snapshot);
    const element = await mount({ load });
    await vi.waitFor(() => expect(element.querySelectorAll("svg g")).toHaveLength(2));
    ...
    expect(load).toHaveBeenLastCalledWith(expect.objectContaining({ version: 2 }));
  });

  it("shows an empty state for boards without widgets and a fallback without a loader", async () => {
    const empty = await mount({ load: async () => ({ ...snapshot, widgets: [] }) });
    await vi.waitFor(() => expect(empty.textContent).toContain("No widgets yet"));

    const unavailable = await mount({ load: undefined });
    await vi.waitFor(() => expect(unavailable.textContent).toContain("Preview unavailable"));
