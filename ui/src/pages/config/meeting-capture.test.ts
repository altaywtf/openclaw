...
describe("curated meeting capture", () => {
  ...
  it("explains a rejected daily ID without hiding the custom ID or changing the draft", async () => {
    const { page, runtimeConfig, original } = await mount({ startDiagnostic: "id-conflict" });
    expect(page.textContent).toContain("Choose a new unused custom ID");
    expect(page.textContent).toContain("completed captures cannot resume");
    click(page, "Edit source 1");
    await page.updateComplete;
  ...
  it("edits through the shared draft without dropping locators or a custom session ID", async () => {
    ...
    expect(navigate).toHaveBeenCalledWith("transcripts");
  });
