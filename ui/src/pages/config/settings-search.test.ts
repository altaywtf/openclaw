...
describe("findSettingsSearchBlocks", () => {
  it("finds the meeting library separately from its Communications capture settings", () => {
    ...
    expect(search("meeting notes")).toContainEqual(
      expect.objectContaining({ routeId: "transcripts" }),
    );
    ...
    expect(matches).toEqual([
      {
        ...
        hash: "#settings-communications-meeting-capture",
        searchText:
          "Meeting capture Choose which sources may save durable meeting notes on this Gateway. Auto-start sources recording transcription meetings autoStart",
      },
      {
        routeId: "ai-agents",
        label: "Tools",
