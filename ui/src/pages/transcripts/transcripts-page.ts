...
class TranscriptsPage extends OpenClawLightDomElement {
  ...
  @state() private readerTab: "text" | "summary" = "text";
    ...
    if (changed.has("routeSearch")) {
      ...
      for (const key of [...TRANSCRIPT_FILTER_KEYS, "find"]) {
        if (
          ...
          (key === "find" && previous.get("selected") !== next.get("selected"))
        ) {
          this.drafts[key] = next.get(key) ?? "";
        }
      }
      if (
        previous.get("selected") !== (this.selection.selector || null) ||
  ...
  private navigate(patch: Record<string, string | null>) {
    ...
    this.requestUpdate();
    this.context.navigate("transcripts", {
...
export const transcriptsPageComponent = {
  header: true,
  render: (search: unknown) =>
    html`<openclaw-transcripts-page
      .routeSearch=${typeof search === "string" ? search : ""}
    ></openclaw-transcripts-page>`,
};

if (!customElements.get("openclaw-transcripts-page")) {
  customElements.define("openclaw-transcripts-page", TranscriptsPage);
}
