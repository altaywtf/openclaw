...
function mount(request: ReturnType<typeof vi.fn>, search = "", scopes = ["operator.admin"]) {
  ...
  const page = document.createElement("openclaw-transcripts-page") as TestPage;
...
describe("meeting transcript library", () => {
  ...
  it("preserves reader search through pagination, tabs, and export updates, then submits and clears it", async () => {
    ...
    const { page } = mount(request, "?selected=meeting");
    ...
    page.querySelector<HTMLElement>("#transcript-reader-tab-summary")!.click();
    await page.updateComplete;
    page.querySelector<HTMLElement>("#transcript-reader-tab-text")!.click();
    await page.updateComplete;
    ...
    page.routeSearch = "?selected=other&find=history";
    await vi.waitFor(() =>
      expect(page.querySelector<HTMLInputElement>('input[name="find"]')?.value).toBe("history"),
    );
  });

  it("sends bounded filters, resets opaque paging when filters change, and opens the selected reader", async () => {
  ...
  it("ignores stale reader pages after selection changes, then appends matching search pages", async () => {
    ...
    const { page } = mount(request, "?selected=old");
    await vi.waitFor(() =>
      expect(request).toHaveBeenCalledWith(
        "transcripts.get",
    ...
    page.routeSearch = "?selected=new&find=match";
  ...
  it("clears private reader content on disconnect and does not restore a late response", async () => {
    ...
    const { page, snapshot, notify } = mount(request, "?selected=meeting");
    await vi.waitFor(() =>
  ...
  it.each([
    ["empty", null, "No transcripts found"],
  ...
  it("does not request archive content without read permission", async () => {
    const request = vi.fn();
    const { page } = mount(request, "?selected=private", ["operator.approvals"]);
    await page.updateComplete;
  ...
  it.each(["selection", "client", "epoch", "same-args retry"])(
    ...
    async (replacement) => {
      ...
      const { page, snapshot, notify } = mount(request, "?selected=old");
      await vi.waitFor(() => expect(readCount).toBe(1));
      if (replacement === "selection") {
        page.routeSearch = "?selected=new";
  ...
  it.each(["success", "denial"])(
    ...
    async (outcome) => {
      ...
      const { page } = mount(request, `?selected=${encodeURIComponent(meetingEntry.selector)}`);
      await vi.waitFor(() => expect(page.textContent).toContain("Keep the reader quiet"));
      button(page, "Download Markdown").click();
      await vi.waitFor(() => expect(page.textContent).toContain("Preparing download"));
      page.routeSearch = "?selected=another";
      await page.updateComplete;
      if (outcome === "denial") {
        pending.reject(
  ...
  it("ignores a filtered-list denial from the previous selection", async () => {
    ...
    const { page } = mount(request, "?selected=meeting");
    await vi.waitFor(() => expect(page.textContent).toContain("Keep the reader quiet"));
    page.routeSearch = "?selected=meeting&query=filter";
    await vi.waitFor(() =>
      expect(request).toHaveBeenCalledWith(
        "transcripts.list",
    ...
    page.routeSearch = "?selected=other&query=filter";
    await vi.waitFor(() => expect(page.querySelector(".transcripts-list__entry")).not.toBeNull());
  ...
  it("bounds the reading window while preserving a way back to the beginning", async () => {
    ...
    const { page } = mount(request, "?selected=long-meeting");
