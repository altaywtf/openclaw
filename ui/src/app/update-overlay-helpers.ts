...
export type UpdateStatusRefreshMode = "manual" | "background" | "completion";
...
export function createUpdateStatusRefresher(params: {
  ...
  onStatus: (response: UpdateRestartStatusResponse, mode: UpdateStatusRefreshMode) => void;
  onError: (error: unknown) => void;
}) {
  let generation = 0;
  let manualIsCurrent: (() => boolean) | null = null;
  return async (mode: UpdateStatusRefreshMode = "manual") => {
