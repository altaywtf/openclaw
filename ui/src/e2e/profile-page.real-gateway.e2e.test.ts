    ...
    try {
      ...
      await suite.withPage(
        ...
        async ({ page }) => {
          ...
          // A second connected browser tab changes the profile while the first
          // retains its users.self result. Presence must win even when name disappears.
          const editor = await page.context().newPage();
          await editor.goto(new URL("settings/profile", suite.server.baseUrl).href);
          const input = editor.locator(".identity-name-control input");
          await input.waitFor();
          for (const name of ["Updated Person", ""]) {
            await input.fill(name);
            const save = editor.locator('.identity-name-control button[type="submit"]');
            await expect.poll(() => save.isEnabled()).toBe(true);
            await save.click();
            await expect
              .poll(() => page.locator(".profile-hero__name").textContent())
              .toBe(name || authenticatedUser);
          }
          if (proofDir) {
            await page.screenshot({
              animations: "disabled",
              fullPage: true,
              path: path.join(proofDir, "02-real-gateway-cleared-name.png"),
            });
          }
        },
      );
