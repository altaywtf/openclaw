...
const enTranscripts = {
  ...
  meetingCapture: {
    ...
    sessionIdHint:
      "Optional custom session ID, usable once per day. Restarting or re-enabling cannot resume it. Leave it empty for generated IDs (recommended for automation).",
    titleHint:
    ...
    startDiagnostics: {
      ...
      retrying:
        "Capture has not been admitted. The Gateway is retrying startup; check the provider configuration and refresh health.",
      "id-conflict":
        "This ID is already in use or was used today. Choose a new unused custom ID, or leave it empty for generated IDs. Saved notes are unchanged; completed captures cannot resume.",
      "admitted-start-failed":
        "Capture startup failed after the transcript was created. Saved notes are retained and startup will not retry this ID. Check the provider configuration, then choose a new unused ID or leave it empty before restarting.",
      "start-failed":
        "Capture could not start. Check the source and provider configuration, then restart the Gateway and refresh health.",
      ended:
        "Capture ended during startup. Saved notes are retained. Choose a new unused ID or leave it empty before starting another capture.",
    },
    locatorsHint:
    ...
    durationHint:
      "Sources capture continuously unless occupancy mode is enabled. Occupancy mode saves notes after the room empties; generated sessions can reopen within ten minutes. Fixed IDs cannot resume.",
    sttHint:
