...
describe("transcript English loading", () => {
  it.each([
    { surface: "library", load: () => import("../pages/transcripts/view.ts") },
    ...
    expect(capture.searchKeys.map((key) => manager.t(key))).toEqual([
      "Choose which sources may save durable meeting notes on this Gateway.",
      "Auto-start sources",
    ]);
    expect(manager.t("tabs.transcripts")).toBe("Transcripts");
