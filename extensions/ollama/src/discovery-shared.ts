...
import { isHostedOllamaCloud, isLocalOllamaBaseUrl } from "../local-model-api.js";
...
export { isLocalOllamaBaseUrl } from "../local-model-api.js";
  ...
  if (!isLocalOllamaBaseUrl(params.baseUrl)) {
    if (resolvedDiscoveryApiKey) {
      return {
        ...
        profileId: params.resolvedAuth.profileId,
      };
    }
...
export async function resolveOllamaDiscoveryResult(params: {
  ...
  const { apiKey, discoveryApiKey, profileId } = auth;
  ...
  return await runLiveProviderCatalog({
    ...
    run: async () => {
      ...
      try {
        provider = await params.buildProvider(configuredBaseUrl, {
          quiet,
