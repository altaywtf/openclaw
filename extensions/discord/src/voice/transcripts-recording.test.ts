  ...
  ({
      ...
      async ({ excluded, dispatch }) => {
        ...
        try {
          for (let marker = 1; marker <= 3; marker++) {
            ...
            await vi.waitFor(() => expect(f.sink).toHaveBeenCalledTimes(marker));
            await f.entry.processingQueue;
    ...
    it.each([
      ...
      async ({ dispatch, middle }) => {
        ...
        const savedTexts =
          middle === "failure" || middle === "empty" ? [texts[0], texts[2]] : texts;
    ...
    it("uses prepared audio upload caps and packet timestamps across speech gaps", async () => {
      resolveAudioInputBudgetMock.mockResolvedValueOnce({ enabled: true, maxBytes: 192_044 });
