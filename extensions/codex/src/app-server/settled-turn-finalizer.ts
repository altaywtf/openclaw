...
export async function runCodexSettledTurnFinalization(
  ...
  if (nativeProvider !== "codex" && nativeProvider !== "openai") {
    // Captured native identity owns the side turn; only its prepared provider may supply the key.
    const preparedProvider = hostAuthPlan?.modelRoute?.provider ?? attempt.model.provider;
    if (preparedProvider.trim().toLowerCase() !== nativeProvider.trim().toLowerCase()) {
      throw new Error(
