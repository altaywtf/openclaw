...
describe("custom Codex provider binding", () => {
  ...
  it("rejects endpoint drift when revalidating a reused server", () => {
    const provider = { ...nativeProvider };
    const config = { ...effectiveConfig(), model_providers: { [binding.provider]: provider } };
    assertCodexCustomProviderEffectiveConfig(binding, config);
    provider.base_url = "https://other.example/v1";
    expect(() => assertCodexCustomProviderEffectiveConfig(binding, config)).toThrow(/endpoint/);
  });
