...
async function runBoundedCodexAppServerTurnInWorkspace(
  ...
  if (
    ...
    // Private finalization cannot inherit the agent's tools/config. Project only
    // its prepared route; the key stays in the process-owned environment.
  ...
  try {
    params.assertCurrent?.();
    // The first-party catalog cannot enumerate an operator's custom models.
    // Explicit text-only follow-ups retain the prepared workload selection;
    // native inference still rejects models the custom endpoint cannot serve.
