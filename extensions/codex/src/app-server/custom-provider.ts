...
const RESERVED_PROVIDER_IDS = new Set(["codex", "openai", "constructor", "prototype", "__proto__"]);
...
export function normalizeCodexCustomProviderId(value: unknown): string | undefined {
  ...
  return /^[a-z0-9][a-z0-9_-]*$/.test(provider) && !RESERVED_PROVIDER_IDS.has(provider)
    ? provider
    : undefined;
}
