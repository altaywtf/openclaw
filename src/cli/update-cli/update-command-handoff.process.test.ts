...
// Fresh Node callers borrow the same sealed runtime graph prepared by the test runner.
const handoffCallerUrl = resolveRuntimeWorkerUrl(triageTestRuntimeEntrypoints.updateHandoff);
...
import {handoffUpdateFromGateway} from ${JSON.stringify(handoffCallerUrl.href)};
try {
...
process.stdin.once('end',()=>{if(child.exitCode===null&&child.signalCode===null)child.kill('SIGKILL');process.stdin.destroy();});`,
    ...
    try {
      ...
      if (mode === "transfer") {
        ...
        expect(JSON.parse(result.stdout), result.stderr).toEqual({ code: "SQLITE_CANTOPEN" });
