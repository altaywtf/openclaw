...
describe("runCapability video provider wiring", () => {
  it.each([undefined, false, true])(
    "preserves processing evidence (%s) while safely truncating video text",
    async (verified) => {
      await withVideoFixture("openclaw-video-utf16-output", async ({ ctx, media, cache }) => {
            ...
            [
              "moonshot",
              {
                ...
                describeVideo: async (req) => ({
                  ...
                  ...(verified === undefined
                    ? {}
                    : { processing: { mode: "agentic" as const, verified } }),
                }),
              },
            ],
          ]),
        ...
        expect(output.processing).toEqual(
          verified === undefined ? undefined : { mode: "agentic", verified },
        );
      });
    },
  );
