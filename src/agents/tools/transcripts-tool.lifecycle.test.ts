...
import { TranscriptSessionConflictError, TranscriptsStore } from "../../transcripts/store.js";
...
describe("transcript capture ownership", () => {
  ...
  it.each(["terminal", "rejected", "thrown"] as const)(
    "rejects same-millisecond admission after a %s startup and fences old callbacks",
    async (outcome) => {
      ...
      const summary = await h.store.readSummary(first);
      await expect(h.start()).rejects.toBeInstanceOf(TranscriptSessionConflictError);
      expect(start).not.toHaveBeenCalled();
      expect(await h.session()).toEqual(first);
      expect(await h.store.readSummary(first)).toEqual(summary);
  ...
  it.each([
    { action: "start", elapsed: 0, historical: false },
    { action: "import", elapsed: 0, historical: false },
    { action: "start", elapsed: 5_000, historical: false },
    { action: "import", elapsed: 5_000, historical: false },
    { action: "start", elapsed: 5_000, historical: true },
    { action: "import", elapsed: 5_000, historical: true },
    { action: "start", elapsed: -5_000, historical: true },
    { action: "import", elapsed: -5_000, historical: true },
  ] as const)(
    "rejects new $action after $elapsed ms without changing the archive (historical=$historical)",
    async ({ action, elapsed, historical }) => {
      vi.useFakeTimers({ toFake: ["Date"] });
      const at = Date.parse("2026-07-01T10:00:00.000Z");
      vi.setSystemTime(at);
      const h = harness();
      await h.start();
      await h.requests[0]!.onUtterance({
        id: "original",
        text: "Preserved decision",
        final: true,
        speaker: { id: "speaker", label: "Speaker" },
        metadata: { provenance: "original" },
      });
      await h.execute({ action: "stop", sessionId: "notes" });
      const original = await h.session();
      const artifacts = await h.store.materializeSessionArtifacts(original, "all");
      const { db } = openOpenClawStateDatabase({
        env: { ...process.env, OPENCLAW_STATE_DIR: h.stateDir },
      });
      if (historical) {
        db.prepare("UPDATE meeting_transcript_sessions SET selector = ? WHERE session_id = ?").run(
          "2026-07-01/historical",
          original.sessionId,
        );
        await fs.rm(artifacts.sessionDir, { recursive: true });
      }
      const snapshot = async () => ({
        session: await h.session(),
        utterances: await h.store.readUtterancesForSession(original),
        summary: await h.store.readSummary(original),
        rows: [
          "meeting_transcript_sessions",
          "meeting_transcript_utterances",
          "meeting_transcript_summaries",
        ].map((table) => db.prepare(`SELECT * FROM ${table} WHERE session_id = ?`).all("notes")),
        exports: historical
          ? await fs.readdir(artifacts.sessionDir).catch((error: unknown) => {
          ...
          : await Promise.all(
              (await fs.readdir(artifacts.sessionDir))
                .toSorted()
                .map(async (name) => [
                  name,
                  await fs.readFile(path.join(artifacts.sessionDir, name), "utf8"),
                ]),
            ),
      });
      const before = await snapshot();
      const importing = vi.spyOn(manualTranscriptSourceProvider, "importTranscript");
      vi.setSystemTime(at + elapsed);
      const other = createTranscriptsTool({
        stateDir: h.stateDir,
        agentId: "other",
        config: { transcripts: { enabled: true } },
        caller: { kind: "operator", source: "local" },
      ...
      await expect(
        other.execute("collision", {
          action,
          sessionId: "notes",
          title: "Replacement",
          providerId: action === "start" ? "capture" : "manual-transcript",
          accountId: "other",
          meetingUrl: "https://example.test/other",
          transcript: "New: must not append",
        }),
      ).rejects.toMatchObject({ name: "TranscriptSessionConflictError", cause: expect.any(Error) });
      expect(h.provider.start).toHaveBeenCalledOnce();
      expect(importing).not.toHaveBeenCalled();
      expect(isTranscriptSessionStarting("notes")).toBe(false);
      await expect(h.execute({ action: "status" })).resolves.toMatchObject({
        details: { active: [], pendingFinalization: [] },
      });
      expect(await snapshot()).toEqual(before);
      // A rejected claim cannot poison a later, independently generated admission.
      await expect(h.execute({ action: "start", providerId: "capture" })).resolves.toMatchObject({
        details: { sessionId: expect.stringMatching(/^transcript-/) },
      });
      expect(await snapshot()).toEqual(before);
    },
  );

  it("admits only one competing manual import at the same timestamp", async () => {
    vi.useFakeTimers({ toFake: ["Date"] });
    vi.setSystemTime(new Date("2026-07-01T10:00:00.000Z"));
    const h = harness();
    const importing = vi.spyOn(manualTranscriptSourceProvider, "importTranscript");
    const attempts = await Promise.allSettled(
      ["First", "Second"].map((text) =>
        h.createTool().execute("competing-import", {
          action: "import",
          sessionId: "notes",
    ...
    expect(attempts.filter((result) => result.status === "rejected")).toMatchObject([
      { reason: { name: "TranscriptSessionConflictError", cause: expect.any(Error) } },
    ]);
    ...
    expect((await h.store.readSummary(original)).summary?.utteranceCount).toBe(1);
    expect(await h.store.listSessionEntries()).toHaveLength(1);
    expect(isTranscriptSessionStarting("notes")).toBe(false);
  });
