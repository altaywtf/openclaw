...
import { hoisted, preparedModelRuntime } from "./simple-completion-runtime.test-support.js";
import type { SimpleCompletionModelResolver } from "./simple-completion-scope.js";
import { makeProviderModelFixture } from "./test-helpers/provider-model-fixture.js";

// Register fixture mocks before loading the runtime under test.
const { prepareSimpleCompletionModel, acquireSimpleCompletionModelForAgent } =
...
describe("prepareSimpleCompletionModel", () => {
  ...
  it("keeps prepared direct auth on its API route without rediscovering a failed profile", async () => {
    hoisted.reconcileAuthProfileQuotaBlocksMock.mockRejectedValue(
      new Error("Subscription quota probe unavailable"),
    );
