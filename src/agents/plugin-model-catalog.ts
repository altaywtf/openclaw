...
export type PluginModelCatalogMetadataSnapshot = Pick<PluginMetadataSnapshot, "owners"> & {
  plugins?: readonly Pick<
    PluginMetadataSnapshot["plugins"][number],
    "id" | "activation" | "modelCatalog" | "providerEndpoints"
  >[];
  index?: {
...
export function filterGeneratedPluginModelCatalogProviders<T>(params: {
  ...
  const catalogPlugin = params.pluginMetadataSnapshot.plugins?.find(
    (plugin) => plugin.id === params.catalogPluginId,
  );
  if (catalogPlugin?.activation?.onStartup === false) {
    // Deferred plugin catalogs are valid only after the current runtime explicitly activates them.
    // Manifest ownership alone must not revive a stale local-server inventory.
    const hasAvailableProvider = Object.keys(params.providers).some((providerId) =>
