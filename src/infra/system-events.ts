...
import { releaseSessionBackgroundTarget } from "../sessions/session-background-custody.js";
...
function releaseSystemEvent(event: SystemEvent): void {
...
function enqueueOwnedSystemEventEntry(
  ...
  if (options.replace) {
    ...
    matching.forEach(releaseSystemEvent);
  ...
  cloneSystemEventOwner(options, event);
  recordSystemEventOwner(event, normalizedOwnerAgentId);
  entry.queue.push(event);
  if (entry.queue.length > MAX_EVENTS) {
    releaseSystemEvent(entry.queue.shift()!);
  }
  return event;
}
...
function drainSystemEventsWith<T>(sessionKey: string, project: (event: SystemEvent) => T): T[] {
  ...
  entry.queue.forEach(releaseSystemEvent);
