...
import { runAgentCleanupStep, withAgentCleanupOutcome } from "../agents/run-cleanup-timeout.js";
...
describe("runUpdateRepairLoop", () => {
  ...
  it.each(["error", "timeout"])(
    "validates an ordinary %s turn with confirmed cleanup before accepting repair",
    async (status) => {
      runtime.runUpdateRepairTurn.mockResolvedValueOnce(turnResult("Partial repair", 1, status));
      const validate = vi.fn().mockResolvedValueOnce(unhealthy(-1)).mockResolvedValueOnce(healthy);
      const cleanupOutcomes: string[] = [];
      const result = await withAgentCleanupOutcome(
        () => runUpdateRepairLoop(params(validate)),
        async (outcome) => {
          cleanupOutcomes.push(outcome);
        },
      );
      expect(cleanupOutcomes).toEqual(["closed"]);
      expect(result.status).toBe("repaired");
      expect(validate).toHaveBeenCalledTimes(2);
      expect(result.attempts).toHaveLength(1);
    },
  ...
  it.each([
    { cleanup: "failure", validation: healthy },
    { cleanup: "timeout", validation: unhealthy(-1) },
  ])("fences the repair lifecycle after cleanup $cleanup", async ({ cleanup, validation }) => {
    // A failed cleanup poisons this module's owner until process exit; instantiate
    // another process-local owner without adding a production reset hook.
    vi.resetModules();
    const { runUpdateRepairLoop: runIsolatedRepair } = await import("./update-repair-agent.js");
    const deferred = createDeferred();
    runtime.runUpdateRepairTurn.mockImplementationOnce(async () => {
      await runAgentCleanupStep({
        runId: "repair-run",
        sessionId: "repair-session",
        step: "repair-resource",
        timeoutMs: 1,
        cleanup: () =>
          cleanup === "timeout" ? deferred.promise : Promise.reject(new Error("cleanup failed")),
        log: { warn: vi.fn() },
      });
      return turnResult("Partial repair", 1, "error");
    });
    const validate = vi.fn().mockResolvedValueOnce(unhealthy(-2)).mockResolvedValue(validation);
    const events: string[] = [];
    try {
      const cleanupOutcomes: string[] = [];
      const result = await withAgentCleanupOutcome(
      ...
      expect(result.attempts).toHaveLength(1);
      expect(cleanupOutcomes).toEqual(["uncertain"]);
      expect(events).toEqual([
        "validation",
        "route-selected",
        "turn-started",
      ...
      const nextValidate = vi.fn().mockResolvedValue(healthy);
      expect(await runIsolatedRepair(params(nextValidate))).toMatchObject({
        status: "unavailable",
      });
      expect(nextValidate).not.toHaveBeenCalled();
      expect(runtime.runUpdateRepairTurn).toHaveBeenCalledOnce();
    } finally {
      deferred.resolve();
