...
const failureSchema = z.strictObject({
  ...
  installationRoot: z.string().max(4096).optional(),
...
export async function continueTriageInFreshProcess(params: {
  ...
  const armShutdown = () => {
    shutdown ??= setTimeout(() => {
      ...
      forced = true;
      try {
        lease = store.revoke(lease, true) ?? lease;
      } catch {
...
> {
  ...
  const cancel = () => {
    ...
    controller.abort(new Error("automatic triage cancelled: live owner or claim lost"));
  ...
  try {
    const { failure, installRoot, owner } = continuationSchema.parse(
