  ...
  it.each(["metadata", "bytes"] as const)(
    ...
    async (operation) => {
      await withEnvAsync({ OPENCLAW_STATE_DIR: path.join(temp, "placement-state") }, async () => {
        ...
        const inbound = path.join(resolveStateDir(), "media", "inbound", "cloud-upload.png");
        await fs.mkdir(path.dirname(inbound), { recursive: true });
        await fs.writeFile(inbound, PNG);
        const reference = "media://inbound/cloud-upload.png";
        const metadata = await request(reference);
        expect(metadata.payload).toMatchObject({ available: true });
        expect(
          (
