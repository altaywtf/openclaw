...
it.each([false, true])(
  "keeps a fixed-ID capture through a real title config commit (pending=%s), then rejects same-day restart",
  async (pending) => {
      ...
      async () => {
        await withPluginRuntimeRegistryScope(registry, async () => {
          ...
          try {
            ...
            // Real time has advanced since admission; a same-millisecond upsert cannot hide the collision.
            expect(Date.now()).toBeGreaterThan(Date.parse(admitted.startedAt));
            service = createTranscriptsAutoStartService({
              stateDir,
              config: current,
              agentId: "notes",
            ...
            await vi.waitFor(async () =>
              expect(
                (await readTranscriptLibraryStatus(store, current)).configuredSources[0],
              ).toMatchObject({ state: "not-active", startDiagnostic: "id-conflict" }),
            );
            expect(requests).toHaveLength(1);
            ...
            for (const transcripts of [
              { ...current.transcripts, enabled: false },
              { ...current.transcripts, autoStart: [] },
            ]) {
              await service.stop();
              service = createTranscriptsAutoStartService({
                stateDir,
                config: { ...current, transcripts },
                agentId: "notes",
                logger,
              });
              service.start();
            ...
            // Generated IDs remain an independent automation sibling, not a
            // substitution in the fixed-ID title/restart proof above.
