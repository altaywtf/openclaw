...
export function createDispatchReplyOperationCoordinator(params: {
  ...
  const ensureDispatchReplyOperation = async (
    ...
    resolveBackgroundTurn(params.replyOptions)?.claim?.(
      admission.operation,
      params.operationSessionStoreEntry.storePath,
  ...
  const getReplyOptions = (): DispatchFromConfigParams["replyOptions"] => {
    ...
    // Admission can follow an owned compaction successor while waiting. Preserve
    // that exact pin when the resolver initializes the admitted session.
    const expectedExistingSessionId =
      resolveBackgroundTurn(params.replyOptions) &&
      params.replyOptions?.expectedExistingSessionId !== undefined
        ? (dispatchReplyOperation?.sessionId ?? params.replyOptions.expectedExistingSessionId)
        : params.replyOptions?.expectedExistingSessionId;
    return {
      ...params.replyOptions,
