...
/** Keyset reads use the existing start-time index; filters never turn into ownership. */
export function* iterateTranscriptReadEntries(
  ...
  if (options.startedAfter) {
    query = query.where("started_at", ">=", options.startedAfter);
  }
  if (options.startedBefore) {
    query = query.where("started_at", "<", options.startedBefore);
  }
  if (options.after) {
    ...
    query = query.where((eb) =>
      eb.or([
        ...
        eb.and([eb("started_at", "=", after.startedAt), eb("session_id", ">", after.sessionId)]),
      ]),
    );
  }
  if (options.query) {
