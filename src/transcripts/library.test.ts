...
describe("transcript library SQLite reads", () => {
  ...
  it("exports full canonical content even when the stored basic summary covers only a tail", async () => {
    const { store, stateDir } = fixture();
    ...
    await store.writeSummary(
      summarizeTranscripts({ session: target, utterances: [{ text: "Action: verify downloads" }] }),
      target,
    );
