...
describe("transcript library capture health", () => {
  ...
  it("bounds settings rows with explicit omissions and treats missing metadata as unknown", async () => {
    ...
    const cfg: OpenClawConfig = {
      transcripts: {
        autoStart: Array.from({ length: 102 }, (_, index) => ({ providerId: `missing-${index}` })),
      },
    };
    const scoped = createPluginMetadataSnapshot({
      config: cfg,
    ...
    const lease = installTemporaryCurrentPluginMetadataSnapshot(
      { ...scoped, pluginIds: ["limited-scope"] },
      { config: cfg },
    );
    try {
      const result = await readTranscriptLibraryStatus(store, cfg);
      ...
      expect(result.omitted).toMatchObject({ configuredSources: 2, providers: expect.any(Number) });
      expect(
        result.providers
