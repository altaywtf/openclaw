...
describe("configured transcript source provenance", () => {
      ...
      try {
        ...
        await vi.waitFor(async () =>
          expect((await f.read()).configuredSources[0]).toMatchObject({
            startDiagnostic: fixed ? "admitted-start-failed" : "retrying",
            state: fixed ? "not-active" : "unknown",
          }),
        );
        const request = start.mock.calls[0]![0];
        ...
        const attempts = fixed ? 1 : Math.min(failures + 1, 12);
        ...
        await expect(f.store.readSession(admitted.sessionId)).resolves.toEqual(before);
        expect(await f.store.listSessionEntries()).toHaveLength(1);
        const configured = (await f.read()).configuredSources[0];
        expect(configured?.state).toBe(!fixed && failures < 12 ? "armed" : "not-active");
        if (!fixed && failures < 12) {
          expect(configured).not.toHaveProperty("startDiagnostic");
        } else {
          expect(configured?.startDiagnostic).toBe(
            fixed ? "admitted-start-failed" : "start-failed",
          );
        }
        await vi.advanceTimersByTimeAsync(65_000);
        expect(start).toHaveBeenCalledTimes(attempts);
  ...
  it.each(
    ...
    async ({ whenOccupied, fault }) => {
      ...
      try {
        ...
        const otherConfig = {
          transcripts: { autoStart: [{ ...room, whenOccupied, sessionId: session.sessionId }] },
        };
        const other = createTranscriptsAutoStartService({ ...f.ctx, config: otherConfig });
        try {
          other.start();
  ...
  it("reports a consumed same-timestamp admission without retrying or retaining a reservation", async () => {
    vi.useFakeTimers({ toFake: ["Date", "setTimeout", "clearTimeout"] });
    vi.setSystemTime(new Date("2026-07-01T10:00:00.000Z"));
    const f = fixture({ transcripts: { autoStart: [{ ...room, sessionId: "daily" }] } });
    const start = vi.fn(f.provider.start!);
    f.provider.start = start;
    await f.start({ ...room, sessionId: "daily", title: "Original" });
    await f.tool.execute("stop", { action: "stop", sessionId: "daily" });
    const original = await f.store.readSession("daily");
    const service = createTranscriptsAutoStartService(f.ctx);
    try {
      service.start();
      await vi.waitFor(async () =>
      ...
      await vi.advanceTimersByTimeAsync(65_000);
      expect(start).toHaveBeenCalledOnce();
      expect(readTranscriptCaptureSnapshot()).toEqual([]);
      expect(await f.store.readSession("daily")).toEqual(original);
    } finally {
      await service.stop();
    }
    expect((await f.read()).configuredSources[0]).not.toHaveProperty("startDiagnostic");
